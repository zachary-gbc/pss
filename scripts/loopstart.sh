...
# bash /home/pi/scripts/pss/loopstart.sh L-1 1 1
# bash /home/pi/scripts/pss/loopstart.sh G-1-P 1 1
...
echo "MESSAGE $datetime: Loop Start ($vars)" >> /home/pi/log/pss/$log.log

function checkvariable {
  vartype=${1:0:2}
  case $vartype in
    "L-"|"G-") type=${1:0:1}; number=${1:2}; loopgraphic=$1 ;;
    "PS") bash /home/pi/scripts/pss/tvpower.sh $1 ;;
    "IS") bash /home/pi/scripts/pss/tvinput.sh $1 ;;
    "DM")
      if [[ $1 != "DM-0" ]]
      then
        minutes=${1:3}
        echo "/bin/bash /home/pi/scripts/pss/loopstop.sh $vars &" | at now + $minutes minutes
      fi
  esac
}
...
if [[ $loopgraphic != "L-0" ]]
then
  echo "MESSAGE $datetime: Starting $message ($loopgraphic)" >> /home/pi/log/pss/$log.log
  if [[ $omxorvlc == "o" ]]
  then
  ...
  curl -Ss "http://$database_ip/pss/other/dbupdate.php?type=locationstatus&device=$mac&loop=$loopgraphic" >> /home/pi/log/pss/$log.log
