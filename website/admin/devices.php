...
if(isset($_POST['submit']))
{
    ...
    while(isset($_POST["id$x"]))
    {
        ...
        $update="UPDATE Devices SET Dev_Name='$name', Dev_Type='$type', Dev_MAC='$mac', Dev_IP='$ip', Dev_LocName='$locname', Dev_RoomBuilding='$roombuilding', Dev_Orientation='$orientation', Dev_OMXorVLC='$omxorvlc' WHERE (Dev_ID='$id')";
    ...
    if(isset($_POST['newdev']) && trim($_POST['newdev']) != "")
    {
        ...
        $insert="INSERT INTO Devices(Dev_Name, Dev_Type, Dev_LocName, Dev_RoomBuilding) VALUES('$newdev', 'Unknown', 'Unknown', 'Unknown')";
...
while($row = mysqli_fetch_array($rs))
{
    ...
    $table.=("<td>" . date("m/d/Y h:i a",strtotime($row['Dev_PSSGHUpdate'])) . "</td>\n");
    $table.=("<td>" . date("m/d/Y h:i a",strtotime($row['Dev_CSDBConf'])) . "</td>\n");
    $table.=("<td><input type='checkbox' name='delete$x' /></td>\n");
    $table.=("</tr>\n");
}

$roomorbuilding="SELECT Var_Value FROM Variables WHERE (Var_System='$system') AND (Var_Name='RoomOrBuilding')"; $roombuilding="Room";
