...
<?php
$processing="SELECT Var_Value FROM Variables WHERE (Var_System='$system') AND (Var_Name='Background-Processing')"; $backgroundprocessing="1";
...
elseif(isset($_POST['submit']))
{
    ...
    if($id == "new")
    {
    $insert="INSERT INTO pss_Graphics (Gr_Name, Gr_Category, Gr_UpdateDateTime, Gr_Delete) VALUES('$newname', '$category', now(), '$delete')";
    ...
    {
        $getid="SELECT Gr_ID FROM pss_Graphics WHERE (Gr_Name='$newname') AND (Gr_Category='$category') ORDER BY Gr_ID";
    ...
    {
    $update="UPDATE pss_Graphics SET Gr_Name='$newname', Gr_Category='$category', Gr_UpdateDateTime=now(), Gr_Converted='N', Gr_Delete='$delete' WHERE (Gr_ID='$id')";
...
$data="SELECT * FROM pss_Graphics WHERE (Gr_ID='$id')";
...
if($id != "new")
{
    ...
    $aloops="SELECT Lop_ID, Lop_Name, Lop_Orientation FROM pss_Loops WHERE (Lop_Type='Automatic') ORDER BY Lop_Name";
...
else
{
    ...
    $dbcategories="SELECT Gr_Category FROM pss_Graphics GROUP BY Gr_Category ORDER BY Gr_Category";
...
$graphics="SELECT * FROM pss_Graphics WHERE (Gr_Delete='N') $graphicfilter ORDER BY Gr_Category, Gr_Name"; $oldcat=""; $table="";$x=0;
