...
$allmonths="<option value='0'></option><option value='1'>January</option><option value='2'>February</option><option value='3'>March</option>";
$allmonths.="<option value='4'>April</option><option value='5'>May</option><option value='6'>June</option><option value='7'>July</option>";
$allmonths.="<option value='8'>August</option><option value='9'>September</option><option value='10'>October</option>";
$allmonths.="<option value='11'>November</option><option value='12'>December</option>";
...
if(isset($_GET['graphic']) && isset($_GET['loop']))
{
    ...
    {
    $delete="DELETE FROM pss_AutomaticLoopDates WHERE (AD_Graphic='$graphic') AND (AD_Loop='$loop')";
    ...
    if($values != "")
    {
        $insert="INSERT INTO pss_AutomaticLoopDates(AD_Graphic, AD_Loop, AD_Date, AD_Month, AD_StartDateRange, AD_EndDateRange) VALUES " . substr($values,0,-1) . ";";
        if(!$rs=mysqli_query($db,$insert)) { echo("Unable to Run Query: $insert"); exit; }
    }
    }

    $autodates="SELECT * FROM pss_AutomaticLoopDates WHERE (AD_Graphic='$graphic') AND (AD_Loop='$loop')"; $x=0;
