...
if(isset($_POST['submit']))
{
    ...
    while(isset($_POST["id$x"]))
    {
        ...
        $update="UPDATE pss_Locations SET Loc_Name='$name', Loc_Device='$device', Loc_BuildingRoom='$room', Loc_Orientation='$orientation' WHERE (Loc_ID='$id')";
    ...
    if(isset($_POST['newloc']) && trim($_POST['newloc']) != "")
    {
        ...
        $insert="INSERT INTO pss_Locations(Loc_Name, Loc_BuildingRoom, Loc_UpdateDateTime) VALUES('$newloc', 'Unknown', now())";
        if(!mysqli_query($db,$insert)) { echo("Unable to Run Query: $insert"); exit; }
    }
}

$devices="SELECT Dev_Name, Dev_IP FROM pss_Devices ORDER BY Dev_Name"; $deviceslist=array(); $ips=array();
...
$locations="SELECT * FROM pss_Locations ORDER BY Loc_Name"; $table=""; $devices=""; $x=0;
