...
if(isset($_POST['submit']))
{
    ...
    $delete="DELETE FROM pss_LoopGraphics WHERE (LG_Loop='$id')";
    ...
    if($graphic != "")
    {
        $insert="INSERT INTO pss_LoopGraphics(LG_Loop, LG_Graphic, LG_Order) VALUES('$id', '$graphic', '$order')";
    ...
    if($newname != "")
    {
    $check="SELECT * FROM pss_Loops WHERE (Lop_Name='$newname')"; $updatename=true;
    ...
    $updateloop="UPDATE pss_Loops SET Lop_UpdateDateTime=now()$unewname WHERE (Lop_ID='$id')";
    ...
    {
    $delete1="DELETE FROM pss_Loops WHERE (Lop_ID='$id')";
    $delete2="DELETE FROM pss_LoopGraphics WHERE (LG_Loop='$id')";
...
if(isset($_POST['new']) && strlen($_POST['new']) > 1)
{
    ...
    $check="SELECT * FROM pss_Loops WHERE (Lop_Name='$name')";
    if(!$rs=mysqli_query($db,$check)) { echo("Unable to Run Query: $check"); exit; }
    while($row = mysqli_fetch_array($rs)) { echo("Name Already Exists, Choose New Name"); $add=false; }

    if($add == true)
    {
    $insert="INSERT INTO pss_Loops(Lop_Name, Lop_Category, Lop_UpdateDateTime, Lop_Orientation, Lop_Type) VALUES('$name', '$category', now(), '$orientation', '$type')";
...
if(isset($_GET['id']))
{
    ...
    $manualcheck="SELECT Lop_Type FROM pss_Loops WHERE (Lop_ID='$id')";
    ...
    $graphicslist="SELECT * FROM pss_Graphics WHERE (Gr_Delete='N') ORDER BY Gr_Category, Gr_Name";
...
$list="SELECT * FROM pss_Loops ORDER BY Lop_Category, Lop_Type, Lop_Name"; $loops=""; $currentloop=""; $oldcat="";
