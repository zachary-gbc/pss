...
if(isset($_GET['devid'])) { $devid=$_GET['devid']; } elseif(isset($POST['devid'])) { $devid=$POST['devid']; } else { $devid=""; }
        ...
        else
    {
        ...
        $update="UPDATE pss_Schedules SET Sch_Name='$name', Sch_LoopGraphic='$loopgraphic', Sch_RMinute='$minute', Sch_RHour='$hour', Sch_RDOM='$day', Sch_RMonth='$month', Sch_RDOW='$dow', Sch_DurationMinutes='$duration', Sch_ScreenPowerStart='$screenpowerstart', Sch_ScreenPowerEnd='$screenpowerend', Sch_ScreenInputStart='$inputstart', Sch_ScreenInputEnd='$inputend',  Sch_Active='$active', Sch_UpdateDateTime=now() WHERE (Sch_ID='$id')";
        if(!mysqli_query($db,$update)) { echo("Unable to Run Query: $update"); exit; }
    }

            if(isset($_POST["delete$x"])) { $delete="DELETE FROM pss_Schedules WHERE (Sch_ID='$id')"; if(!mysqli_query($db,$delete)) { echo("Unable to Run Query: $delete"); exit; } }
            $x++;
        }
        ...
        if(isset($_POST['otnew']) && trim($_POST['otnew']) != "")
        {
            ...
            $insert="INSERT INTO pss_Schedules(Sch_Name, Sch_Device, Sch_LoopGraphic, Sch_ScreenInput, Sch_OneTimeRecurring, Sch_UpdateDateTime) VALUES('$newschedule', '$devid', '0', '0', 'O', now())";
        ...
        if(isset($_POST['recnew']) && trim($_POST['recnew']) != "")
        {
            ...
            $insert="INSERT INTO pss_Schedules(Sch_Name, Sch_Device, Sch_LoopGraphic, Sch_ScreenInput, Sch_OneTimeRecurring, Sch_UpdateDateTime) VALUES('$newschedule', '$devid', '0', '0', 'R', now())";
            if(!mysqli_query($db,$insert)) { echo("Unable to Run Query: $insert"); exit; }
        }
    }

$locations=""; $orientation=""; $room=""; $locationname=""; $x=0;
$alldevices="SELECT * FROM pss_Devices ORDER BY Dev_RoomBuilding, Dev_LocName";
...
if($devid != "")
{
    ...
    $allloops="SELECT * FROM pss_Loops WHERE (Lop_Orientation='$orientation') ORDER BY Lop_Name";
    ...
    $allgraphics="SELECT * FROM pss_Graphics WHERE (Gr_Delete='N') ORDER BY Gr_Category, Gr_Name";
    ...
    $schedules="SELECT * FROM pss_Schedules WHERE (Sch_Device='$devid') ORDER BY Sch_Active DESC, Sch_Name";
